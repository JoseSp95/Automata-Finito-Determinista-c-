...
int main() {
    ...
    while(preguntar){
        ...
        while(band && i < strlen(cadena)){

            band = false;

            for(j = 0; j < nroEstados; j++){
                if(transiciones[estado][j] == cadena[i]){
                    estado = j;
                    band = true;
                    cout<<" caracter aceptado -> "<<cadena[i]<<" *** "<<endl;
                    i++;
                    break;
                }
            }
        ...
        if(!band){
            cout<<endl<<" CADENA RECHAZADA ";
        }else{
            bool esUnEstadoFinal = false;
            /* buscamos si el ultimo estado es estado FINAL */
            for(i = 0 ;  i < nroEstadosFinales ; i++){
                if( estado == estadosFinales[i] ){
                    esUnEstadoFinal = true;
                    break;
                }
            }
